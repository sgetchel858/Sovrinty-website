...
| s1-single-source  | anim  | S1 | 16/9 | —    | **live** (CSS/JS) | documents drop into three scattered boxes (SharePoint / Box / Shared Drives) → a gold chain draws itself → gold beads stream down it → absorbed into the sealed "Sovrinty" core (caption: connected to your sources). Documents stay in their source boxes — only the gold chain reaches the core, so it reads as connect-in-place, not migration. Plays once on scroll-in; reduced-motion shows the settled frame. `pages/walkthrough.css` (S1 section) + the shared `js/modules/walkthrough-scenes.js` scroll trigger |
...
| s3-sme-desk       | video | S3 | 16/9 | ~20s | approved | SME answers a routed question entirely inside the Teams chat (no modal): bot routes "Do we support SAML SSO with Okta?" → SME types an answer → bot asks to submit → SME types "yes" → bot confirms it's sealed into the knowledge base. Camera holds an ~88px bottom safe-area so the chat clears a video player's control scrim; poster is the opening full-client view (frame 40). No captions (removed). Remotion build in `Videos/Scenes/s3-sme-desk` → `assets/media/{video,poster}/s3-sme-desk.*` |
